...
admissions_10013310 <- read_csv("~/mimic/hosp/admissions.csv.gz") |>
  filter(subject_id == sid) |>
  #select first row, only need demographic information
  slice_head()
...
ggplot() +
  ...
  #select shapes for each procedure 
  scale_shape_manual(
    values = c(15, 16, 17, 3, 6, 7, 8, 10, 13)
    ) +
  #add titles
  labs(
    title = str_c(
  ...
  theme_light() + 
  #edits to legend and aspect ratio
  theme(
  ...
  guides(
    shape = guide_legend(ncol = 1, nrow = 9),
    color = guide_legend(ncol = 1, nrow = 7),
    linewidth = "none"
    ) +
  #edits to y axis labels
  scale_y_continuous(
...
```
Yes, there is an unusually high number of admissions that occur at 0am and 7am, relative to the other hours in the morning. This could be due to opening/closing times of typical hospital hours. Aside from these two unusual hours, admissions are generally low in the morning; however, admissions start to increase in the early afternoon and stay at high numbers during the night. 
