...
```
**Answer:** The output of the bash script above shows a list of gz files in the `hosp` folder that contain the characters "a", "l", or "pa" at the beginning, in long format. This includes the `admissions.csv.gz`, `labevents.csv.gz`, and `patients.csv.gz` files. 

Display the number of lines in each data file using a similar loop. (Hint: combine linux commands `zcat <` and `wc -l`.)

**Answer:** Below are the number of lines in each data file, including the header.  
```{bash}
...
```
There are 431232 rows in the `admissions.csv.gz` file, including the header. 

```{bash}
zcat < ~/mimic/hosp/admissions.csv.gz | awk -F, '{print $1}' | 
grep -v 'subject_id' | sort | uniq | wc -l
```
There are 180733 unique patients in the `admissions.csv.gz` file. 

```{bash}
zcat < ~/mimic/hosp/patients.csv.gz | awk -F, '{print $1}' | 
grep -v 'subject_id' | sort | uniq | wc -l
```
The number of patients listed in the `patients.csv.gz` file is 299712. Thus, the number of unique patients in the `admissions.csv.gz` file does not match the number of patients in the `patients.csv.gz` file. 
...
```
The run time of `zcat < ~/mimic/hosp/labevents.csv.gz | wc -l` was around 18 seconds, while the run time of `wc -l labevents.csv` was around 6 seconds. Although the file size of the compressed file was smaller, it did take longer to run the command. In contrast, the file size of the uncompressed file was large, but the run time for the command was much faster. Thus, there are trade-offs between having a small, compressed file where commands run slower versus a large, uncompressed file where commands run faster. 
...
**Answer:** The `wget -nc` command, when run in terminal, allows the user to download files from the Internet, non-interactively. The `-nc` option prevents the user from overwriting files when a shell redirect is used. 
...
**Answer:** Using `>` will direct the output from the command into the file. Using `>>` will append the output from the command to the file. In this case though, since there was nothing in the `test2.txt` file to begin with, the contents of the two files look the same. 
...
**Answer:** The output includes lines from the middle of the text file `pg42671.txt`. `"$1"`, `"$2"`, and `"$3"` are positional parameters that denote the order that the arguments are passed to the shell script, with `"$1"` representing the first argument, `"$2"` representing the second argument, and so on. We need the first line of the shell script to specify what command is to be run and what positions the arguments should be placed in for the command to run as desired. In this first line, `head` will print out the first few lines of a file, but `-n "$2"` specifies that we want `head -20` to print out the first 20 lines of the file, since `20` is the second argument. `pg42671.txt` is the first argument, so `$1` refers to `pg42671.txt` as the file that we want to select lines from. This output is then piped into `tail -5` since `5` is the third argument, which prints out the last 5 lines of the first 20 lines of the file. 
...
hostname
```
The `hostname` command displays the name of the host system. 

```{bash}
arch
```
The `arch` command displays the architecture type of the system. 

```{bash}
uname -a
```
The `uname -a` command displays information about the system, and the `-a` option is used to display all of the available system characteristics. The information that is displayed includes the kernel name, the hostname, the kernel release date, the kernel version, the machine's hardware name, the hardware platform, and the operating system name. 

```{bash}
uptime
```
The `uptime` command shows how long the system has been running. Specifically, it displays the current time, how long the system has been up for, how many users, and the load averages over the last 1, 5, and 15 minutes, respectively. 

```{bash}
who am i
```
The `who am i` command shows who is logged in on the system. It displays the log-in name and today's date and current time. 

```{bash}
who
```
The `who` command also shows who is logged in on the system, with slightly different information. It displays the log-in name, the terminal name, and the time of log-in. 

```{bash}
w
```
The `w` command also shows who is logged in on the system and what the current activity is. On the first line, it displays the current time, how long the system has been up running for, how many users are logged in, and the load averages over the last 1, 5, and 15 minutes, respectively. It also displays the log-in name, the terminal name, the date of log-in, and how long the system has been idle for. 
...
last | head
```
The `last` command shows a list of the last log-ins of users and terminals, starting from the most recent time. This command is piped into the `head` command, which displays only the first few lines of the list. The output displays the log-in name, the terminal name, the start and stop time of each session, and the duration of each session. 
...
time sleep 5
```
The `time sleep 5` command shows that it took 5 seconds to run the `sleep 5` command. The `sleep 5` command suspends execution for 5 seconds. 

```{bash}
history | tail
```
The `history | tail` command, when run in terminal, shows a list of commands that were previously executed, with the `tail` command displaying only the last few lines of the list, which corresponds to the most recent commands executed. 
...
**Answer:**
<p align="center">
<img src="./biostat203b_hw1_q6.png" width="100%"/>
</p>