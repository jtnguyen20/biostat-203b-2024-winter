...
```
There are 180733 unique patients (subtracted 1 to exclude counting the header) in the `admissions.csv.gz` file. 

```{bash}
zcat < ~/mimic/hosp/patients.csv.gz | awk -F, '{print $1}' | sort | uniq | wc -l
```
The number of patients listed in the `patients.csv.gz` file is 299712 (subtracted 1 to exclude counting the header). Thus, the number of unique patients in the `admissions.csv.gz` file does not match the number of patients in the `patients.csv.gz` file. 
