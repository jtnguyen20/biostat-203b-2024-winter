...
labevents_tble <- arrow::open_dataset("labevents_pq/part-0.parquet", 
                                      format = "parquet") |> as_tibble()
...
#define variables used in ggplot 
sid <- 10013310
#gender <- "F"
#patient_demo <- paste("Patient", sid, )

ggplot() +
  geom_segment(data = subset(transfers_tble, subject_id == sid),
               aes(x = intime, xend = outtime, y = 0, yend = 0, 
                   color = careunit)) +
...
ggplot(data = icustays_tble) +
  geom_bar(mapping = aes(x = subject_id)) + 
  labs(
    title = "Distribution of ICU stays by patient", 
    x = "Patient's Subject ID",
    y = "Count"
  ) 
#ERROR: graph not showing bars... too many subject ids?
```
...
```
Most hospital stays are relatively short, spanning a day to a few days. However, there do seem to be some hospital stays that are longer, spanning several days to weeks. The x axis of the graph extends to 300 days, indicating that there is an unusually long hospital stay that lasted around 300 days. 
...
ggplot(data = patients_tble) +
  geom_histogram(binwidth = 1, mapping = aes(x = anchor_age, fill = gender)) + 
  labs(
    title = "Distribution of patient age by patient gender",
    x = "Age",
    y = "Count",
...
```
In general, there is a larger share of young adult patients compared to adult and elderly patients, for both genders. This observation is surprising, as we would assume the opposite with more elderly patients and less young adult patients. However, the oldest elderly patients seem to make up a sizable share of the total patients, likely due to worsening health conditions. Across all ages, there is a larger share of female patients compared to male patients, and this difference is especially prominent for the youngest and oldest age groups. This could be due to multiple factors or a combination of factors, e.g. women may be more likely to seek out medical care or come in more often for female-specific health conditions. 
...
```{r}
ggplot(data = mimic_icu_cohort) +
  geom_boxplot(mapping = aes(x = insurance, y = los)) + 
  labs(
...
```{r}
ggplot(data = mimic_icu_cohort) +
  geom_boxplot(mapping = aes(x = marital_status, y = los)) + 
  labs(
...
```{r}
ggplot(data = mimic_icu_cohort) +
  geom_boxplot(mapping = aes(x = gender, y = los)) + 
  labs(
...
```{r}
ggplot(data = mimic_icu_cohort) +
  geom_point(mapping = aes(x = non_invasive_blood_pressure_systolic, y = los)) +
  labs(
